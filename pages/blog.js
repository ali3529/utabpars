    ...
    <>
      ...
      <div className='container xl:px-[120px] font-[sans]  '>
        ...
        <div className='flex flex-col mt-20'>
          <p className='text-[26px] font-bold text-center  '> وبلاگ</p>
          ...
          <div className='grid  md:grid-cols-3   grid-cols-1  mt-5 '>
            {/* template big */}
            {
              props.blogs.map((item, index) =>
                <div key={index} className='text-right shadow-md rounded-[2.5rem]  shadow-gray-300 py-6 px-3 sm:mx-8'>
                  {/* img */}
                  <div className='py-2'>
                  ...
                  {/* descrip */}
                  <span className='text-gray-800 line-clamp-2 mt-6'> {item.slug}</span>
                </div>)
            }
          </div>
...
export const getStaticProps = async () => {
  let res = await fetch('https://anitaclinic.ir/api/v1/posts', {
    headers: new Headers({
